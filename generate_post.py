...
TOPICS = {
 ...
 "secure-ota": {
  ...
  "curriculum": [
   ("Why OTA Matters: Field Update Economics & Failure Modes","Foundations",["ota","field-updates"]),
   ("A/B (Dual-Slot) Partitioning: Design & Tradeoffs","Update Architectures",["ab-update","partitioning"]),
   ("Single-Slot vs Recovery-Partition Update Schemes","Update Architectures",["recovery-partition"]),
   ("Image Metadata & Manifests: Versioning, Hashes & Signatures","Image & Metadata",["manifest","versioning"]),
   ("Anti-Rollback Policies: Manifest Versioning & Fleet-Wide Enforcement","Rollback & Recovery",["anti-rollback","versioning"]),
   ("A/B Slot Swap Algorithms: Bank Swap vs Scratch-Area Swap","Update Architectures",["swap-algorithm"]),
   ("Delta/Differential Updates: bsdiff, Courgette & Binary Patching","Delta Updates",["delta-update","bsdiff"]),
   ("Compressing Firmware Images: LZ4, zlib & Constrained Flash","Delta Updates",["compression"]),
   ("Update Verification: Signature Checks Before Boot Commit","Image & Metadata",["signature-verification"]),
   ("Boot Confirmation & Health Checks: Marking an Update \"Good\"","Rollback & Recovery",["boot-confirmation","health-check"]),
   ("Automatic Rollback on Boot Failure & Watchdog-Triggered Recovery","Rollback & Recovery",["rollback","watchdog"]),
   ("mender.io & RAUC: Open-Source OTA Framework Internals","Update Architectures",["mender","rauc"]),
   ("AWS IoT & Azure Device Update Fleet OTA Pipelines","Fleet Management",["aws-iot","azure-adu"]),
   ("Staged Rollouts: Canary Groups & Percentage-Based Deployment","Fleet Management",["staged-rollout","canary"]),
   ("Update Scheduling: Bandwidth, Power & Maintenance Windows","Fleet Management",["scheduling","power-budget"]),
   ("Resuming Interrupted Downloads: Chunked Transfer & Integrity","Delta Updates",["chunked-transfer","resumability"]),
   ("Multi-Image Updates: Bootloader, App & Peripheral Firmware Together","Image & Metadata",["multi-image"]),
   ("OTA Over BLE: GATT-Based DFU & Throughput Constraints","Update Architectures",["ble-dfu"]),
   ("OTA Over Cellular: NB-IoT/LTE-M Bandwidth & Cost Considerations","Update Architectures",["cellular-ota"]),
   ("Bricking Prevention: Failsafe Bootloaders & Golden Images","Rollback & Recovery",["failsafe","golden-image"]),
   ("Update Telemetry: Success/Failure Reporting Back to the Fleet","Fleet Management",["telemetry","reporting"]),
   ("Compliance: EU Cyber Resilience Act Update Requirements","Foundations",["cra","compliance"]),
   ("Testing OTA Pipelines: Fault Injection & Power-Loss Simulation","Fleet Management",["fault-injection","testing"]),
   ("Full Review & Project: Building an A/B OTA Pipeline for an STM32","Review",["review","project"]),
  ],
 },
 "embedded-crypto": {
  "title": "Embedded Cryptography & Key Management Daily Log",
  "curriculum": [
   ("Cryptography Fundamentals for Embedded Engineers","Foundations",["crypto-basics"]),
   ("Symmetric Encryption: AES Modes (ECB, CBC, CTR, GCM)","Symmetric Crypto",["aes","block-cipher"]),
   ("AEAD: Authenticated Encryption with ChaCha20-Poly1305","Symmetric Crypto",["aead","chacha20"]),
   ("Stream Ciphers & Why ECB Mode Is a Trap","Symmetric Crypto",["stream-cipher","ecb-pitfalls"]),
   ("Hash Functions: SHA-256, SHA-3 & Collision Resistance","Hashing & Signatures",["sha256","hashing"]),
   ("HMAC: Message Authentication Without Public Keys","Hashing & Signatures",["hmac","mac"]),
   ("Asymmetric Crypto: RSA Fundamentals & Key Sizes","Asymmetric Crypto",["rsa"]),
   ("Elliptic Curve Cryptography: ECDSA & ECDH Explained","Asymmetric Crypto",["ecc","ecdsa"]),
   ("Digital Signatures: Signing & Verifying Firmware Images","Hashing & Signatures",["digital-signature"]),
   ("Key Exchange: Diffie-Hellman & ECDH in Constrained Devices","Asymmetric Crypto",["key-exchange","ecdh"]),
   ("Random Number Generation: TRNG vs PRNG & Entropy Sources","Foundations",["trng","entropy"]),
   ("Key Derivation Functions: HKDF & PBKDF2","Key Management",["kdf","hkdf"]),
   ("Key Storage: Secure Elements, TPMs & OTP Fuses","Key Management",["secure-element","tpm"]),
   ("Key Provisioning: Factory Injection & Per-Device Keys","Key Management",["provisioning","per-device-keys"]),
   ("Key Rotation & Lifecycle Management in the Field","Key Management",["key-rotation","lifecycle"]),
   ("Hardware Crypto Accelerators: CryptoCell, SE05x & ATECC608","Hardware Crypto",["crypto-accelerator","atecc608"]),
   ("mbedTLS & wolfSSL: Embedded TLS Stack Internals","Hardware Crypto",["mbedtls","wolfssl"]),
   ("TLS 1.3 Handshake on a Microcontroller: Constraints & Optimizations","Protocols",["tls13","handshake"]),
   ("DTLS for Constrained UDP-Based IoT Communication","Protocols",["dtls","udp"]),
   ("Certificate Chains & X.509 for Device Identity","Protocols",["x509","pki"]),
   ("PSA Crypto API: Vendor-Neutral Crypto Abstraction","Hardware Crypto",["psa-crypto"]),
   ("Side-Channel Resistance: Constant-Time Crypto Implementations","Advanced",["side-channel","constant-time"]),
   ("Post-Quantum Cryptography: What Embedded Engineers Need to Know Now","Advanced",["post-quantum","pqc"]),
   ("Full Review & Project: Provisioning Per-Device Keys with ATECC608","Review",["review","project"]),
  ],
 },
 "threat-modeling": {
  "title": "Threat Modeling & Secure Coding Standards Daily Log",
  "curriculum": [
   ("Why Firmware Needs Threat Modeling: Attackers vs Bugs","Foundations",["threat-modeling"]),
   ("STRIDE Methodology: Spoofing, Tampering, Repudiation & More","Threat Modeling",["stride"]),
   ("Data Flow Diagrams for Embedded Systems Threat Models","Threat Modeling",["dfd","threat-modeling"]),
   ("Attack Trees: Modeling Adversary Goals & Paths","Threat Modeling",["attack-tree"]),
   ("Trust Boundaries: Identifying Where Attackers Enter","Attack Surface",["trust-boundary"]),
   ("Attack Surface Analysis: UART, JTAG, USB & Debug Ports","Attack Surface",["debug-interfaces","jtag"]),
   ("OWASP Embedded/IoT Top 10: Overview & Mapping to Firmware","Standards",["owasp","top10"]),
   ("Insecure Network Services & Weak Default Credentials","Attack Surface",["default-creds","network-services"]),
   ("Buffer Overflows in C: Stack Smashing & Mitigations","Secure Coding",["buffer-overflow","stack-smashing"]),
   ("Integer Overflows & Underflows in Firmware Arithmetic","Secure Coding",["integer-overflow"]),
   ("Format String & Injection Vulnerabilities in Embedded C","Secure Coding",["format-string","injection"]),
   ("Use-After-Free & Memory Corruption in Bare-Metal Systems","Secure Coding",["uaf","memory-corruption"]),
   ("MISRA C:2012: Rule Categories & Security Rationale","Standards",["misra-c"]),
   ("MISRA C++ & AUTOSAR C++14 for Safety-Critical Firmware","Standards",["misra-cpp","autosar"]),
   ("CERT C Secure Coding Standard: Key Rules for Embedded","Standards",["cert-c"]),
   ("Static Analysis for Security: Coverity, Klocwork & Cppcheck","Secure Coding",["static-analysis"]),
   ("Fuzzing Embedded Firmware: AFL++, libFuzzer & Hardware-in-the-Loop","Secure Coding",["fuzzing","afl"]),
   ("Command Injection & Insecure Deserialization in IoT Protocols","Attack Surface",["command-injection"]),
   ("Denial-of-Service Resilience: Input Validation & Rate Limiting","Secure Coding",["dos-resilience"]),
   ("Supply Chain Threats: Third-Party Libraries & SBOM","Standards",["supply-chain","sbom"]),
   ("IEC 62443: Industrial Cybersecurity Threat Modeling","Standards",["iec62443"]),
   ("Vulnerability Disclosure & CVE Response for Firmware Vendors","Standards",["cve","disclosure"]),
   ("Penetration Testing Embedded Devices: Tools & Methodology","Attack Surface",["pentest"]),
   ("Full Review & Project: Threat Model for a Connected Door Lock","Review",["review","project"]),
  ],
 },
 "hal-patterns": {
  "title": "HAL & Firmware Design Patterns Daily Log",
  "curriculum": [
   ("Why Hardware Abstraction Matters: Coupling & Portability","Foundations",["hal","portability"]),
   ("Layered Firmware Architecture: BSP, HAL, Middleware & App Layers","Layered Architecture",["layered-architecture"]),
   ("Register Abstraction: Memory-Mapped I/O & volatile Correctness","Layered Architecture",["mmio","volatile"]),
   ("Designing a Clean HAL API: Function Pointers vs Structs","C Patterns",["hal-api","function-pointers"]),
   ("The vtable Pattern in C: Simulating Polymorphism for Drivers","C Patterns",["vtable","polymorphism"]),
   ("Opaque Handles & Pimpl-Style Encapsulation in C","C Patterns",["opaque-handle","encapsulation"]),
   ("Dependency Injection in Embedded C: Decoupling Drivers from Logic","C Patterns",["dependency-injection"]),
   ("State Machines for Driver Design: Table-Driven vs Switch-Based","Driver Design",["state-machine"]),
   ("Observer Pattern for Sensor Data & Event Callbacks","Driver Design",["observer-pattern","callbacks"]),
   ("HAL Design in C++: Templates & Zero-Cost Abstraction","C++ Patterns",["cpp-templates","zero-cost"]),
   ("CRTP for Compile-Time Polymorphic Drivers","C++ Patterns",["crtp"]),
   ("Strategy Pattern: Swappable Communication Backends (SPI/I2C/UART)","Driver Design",["strategy-pattern"]),
   ("Factory Pattern for Peripheral Driver Instantiation","C++ Patterns",["factory-pattern"]),
   ("Device Driver Model: Init, Configure, Read/Write, Deinit Contracts","Driver Design",["driver-model"]),
   ("Porting a HAL Across Vendors: STM32 HAL vs MCUXpresso vs nRFx","Portability",["multi-vendor","porting"]),
   ("Zephyr Device Driver Model: device_api & DEVICE_DT_DEFINE","Portability",["zephyr","device-model"]),
   ("Board Support Packages: Structuring Board-Specific Code","Layered Architecture",["bsp"]),
   ("Peripheral Drivers as Reusable Components: Versioning & APIs","Driver Design",["reusability","api-versioning"]),
   ("Error Handling Patterns Across HAL Layers: Codes vs Exceptions","Driver Design",["error-handling"]),
   ("Testing HALs: Mocking Hardware with Fakes & Hardware-in-the-Loop","Portability",["hal-testing","mocking"]),
   ("Interrupt Abstraction: ISR Registration Patterns Across Vendors","Layered Architecture",["isr","interrupts"]),
   ("Migrating a Product Line to a New MCU: A HAL Case Study","Portability",["migration","case-study"]),
   ("Anti-Patterns: God Objects, Leaky Abstractions & Over-Engineering","Driver Design",["anti-patterns"]),
   ("Full Review & Project: Vendor-Agnostic SPI Sensor Driver","Review",["review","project"]),
  ],
 },
 "embedded-cicd": {
  "title": "Modern CI/CD for Embedded Systems Daily Log",
  "curriculum": [
   ("Why Embedded Teams Need CI/CD: From Desk-Testing to Pipelines","Foundations",["cicd","embedded"]),
   ("Reproducible Builds: Pinning Compiler, SDK & Dependency Versions","Build Automation",["reproducible-builds"]),
   ("GitLab CI & Jenkins Pipelines for Multi-Board Firmware Projects","Build Automation",["gitlab-ci","jenkins"]),
   ("Build Caching Strategies: ccache & Artifact Reuse Across Pipelines","Build Automation",["ccache","build-cache"]),
   ("Compiler Optimization Flags: -O2 vs -Os in a CI Pipeline","Build Automation",["compiler-flags","optimization"]),
   ("Semantic Versioning & Changelogs for Firmware Releases","Release",["semver","changelog"]),
   ("Artifact Repositories: Storing & Signing Firmware Binaries","Release",["artifact-repo","signing"]),
   ("Release Trains: Branching Strategies for Firmware Teams","Release",["branching-strategy","release-train"]),
   ("Automated Release Notes Generation from Commit History","Release",["release-notes","automation"]),
   ("Infrastructure as Code for Build Farms: Ansible & Terraform Basics","Build Automation",["iac","ansible"]),
   ("Binary Size Regression Tracking Across Builds","Static Analysis",["binary-size","regression"]),
   ("SBOM Generation & Dependency CVE Scanning in the Pipeline","Static Analysis",["sbom","cve-scanning"]),
   ("Multi-Target Fan-Out Builds Across Product Variants","Build Automation",["multi-target","fan-out"]),
   ("Deployment Gates: Manual Approval Before Field Rollout","Release",["deployment-gate"]),
   ("Canary & Staged Deployment Pipelines for Firmware","Release",["canary","staged-deployment"]),
   ("Metrics & Dashboards: Tracking Build Health & Flaky Tests","Test Automation",["metrics","dashboards"]),
   ("Rollback Pipelines: Automating Recovery from a Bad Release","Release",["rollback","pipeline"]),
   ("Nightly & Long-Running Soak Test Orchestration","Test Automation",["nightly-build","soak-test"]),
   ("Managing Secrets in CI: Signing Keys & Provisioning Credentials","Build Automation",["secrets-management","ci"]),
   ("Cross-Team Pipelines: Coordinating Firmware, Cloud & Mobile Releases","Release",["cross-team","coordination"]),
   ("Chaos Engineering for Firmware Release Pipelines","Release",["chaos-engineering"]),
   ("Compliance Artifacts: Auto-Generating Traceability from CI Runs","Release",["traceability","compliance"]),
   ("Case Study: Building a Zero-Touch Pipeline from Commit to Field","Release",["case-study"]),
   ("Full Review & Project: Release Engineering for a Multi-Board Product Line","Review",["review","project"]),
  ],
 },
 "wireless-protocols": {
  "title": "Low-Power Wireless & Networking Protocols Daily Log",
  "curriculum": [
   ("Wireless Protocol Landscape: Range, Power & Throughput Tradeoffs","Foundations",["wireless","protocol-comparison"]),
   ("BLE Fundamentals: GAP Roles, Advertising & Connections","BLE",["ble","gap"]),
   ("BLE GATT: Services, Characteristics & Attribute Protocol","BLE",["gatt","att"]),
   ("BLE Power Profiling: Connection Intervals & Duty Cycling","BLE",["ble-power","duty-cycle"]),
   ("BLE Mesh: Provisioning, Models & Relay Nodes","BLE",["ble-mesh"]),
   ("BLE Security: Pairing, Bonding & LE Secure Connections","BLE",["ble-security","pairing"]),
   ("IEEE 802.15.4: The Radio Layer Under Thread & Zigbee","Thread & Matter",["802-15-4"]),
   ("Thread Networking: 6LoWPAN, Mesh Routing & Border Routers","Thread & Matter",["thread","6lowpan"]),
   ("Matter Protocol: Application Layer Over Thread & Wi-Fi","Thread & Matter",["matter"]),
   ("Matter Commissioning: Device Onboarding & Fabric Membership","Thread & Matter",["matter","commissioning"]),
   ("Zigbee vs Thread vs Matter: Choosing the Right Mesh Stack","Thread & Matter",["zigbee","comparison"]),
   ("LoRaWAN Architecture: End Devices, Gateways & Network Servers","LoRaWAN",["lorawan"]),
   ("LoRaWAN Classes A/B/C & Adaptive Data Rate","LoRaWAN",["lorawan","adr"]),
   ("Cellular IoT: NB-IoT vs LTE-M Tradeoffs for Battery-Powered Devices","Cellular IoT",["nb-iot","lte-m"]),
   ("PSM & eDRX: Cellular Power-Saving Modes Explained","Cellular IoT",["psm","edrx"]),
   ("Modem AT Commands & Cellular Firmware Integration Patterns","Cellular IoT",["at-commands","modem"]),
   ("Wi-Fi Low Power: Wi-Fi HaLow & Power-Save Modes for IoT","Networking Stack",["wifi-halow","power-save"]),
   ("CoAP: Constrained Application Protocol Over UDP","Networking Stack",["coap"]),
   ("MQTT & MQTT-SN for Constrained Networks","Networking Stack",["mqtt","mqtt-sn"]),
   ("6LoWPAN & IPv6 Header Compression for Constrained Links","Networking Stack",["6lowpan","ipv6"]),
   ("Network Selection Strategy: Choosing a Protocol for Your Product","Foundations",["protocol-selection"]),
   ("Zephyr Networking Stack: Connection Manager & Sockets API","Networking Stack",["zephyr","connection-manager"]),
   ("Interoperability Testing & Certification: Bluetooth SIG, Thread Group","Thread & Matter",["certification","interop"]),
   ("Full Review & Project: Matter-Compatible Sensor Over Thread","Review",["review","project"]),
  ],
 },
 "edge-ai-tinyml": {
  "title": "Edge AI & TinyML Inference Daily Log",
  "curriculum": [
   ("Why Edge AI: On-Device Inference vs Cloud Round-Trips","Foundations",["edge-ai","tinyml"]),
   ("Neural Network Basics for Embedded Engineers","Foundations",["neural-networks","basics"]),
   ("Model Quantization: int8, Fixed-Point & Quantization-Aware Training","Model Prep",["quantization","int8"]),
   ("Model Pruning & Distillation for Constrained Devices","Model Prep",["pruning","distillation"]),
   ("Converting Models: TensorFlow to TFLite to TFLite Micro","Model Prep",["tflite-conversion"]),
   ("TensorFlow Lite for Microcontrollers: Runtime Architecture","TFLite Micro",["tflite-micro","runtime"]),
   ("Building & Flashing a TFLite Micro Inference App","TFLite Micro",["tflite-micro","deployment"]),
   ("Interpreter vs Compiler Approaches: TFLite Micro vs microTVM","TFLite Micro",["microtvm","compiler-approach"]),
   ("CMSIS-NN: Optimized Neural Network Kernels for Cortex-M","CMSIS-NN",["cmsis-nn","cortex-m"]),
   ("Arena Memory Allocation & Tensor Arena Sizing","Memory & Optimization",["tensor-arena","memory"]),
   ("Fixed-Point Arithmetic: Implementing Quantized Math by Hand","Memory & Optimization",["fixed-point","arithmetic"]),
   ("Optimizing Inference Latency: Loop Unrolling & SIMD on Cortex-M","Memory & Optimization",["simd","latency"]),
   ("Model Size vs Accuracy Tradeoffs for Flash-Constrained MCUs","Memory & Optimization",["model-size","tradeoffs"]),
   ("Edge Impulse: End-to-End TinyML Pipeline for Embedded Sensors","Applications",["edge-impulse","pipeline"]),
   ("Keyword Spotting: Always-On Audio Wake-Word Detection","Applications",["keyword-spotting","audio"]),
   ("Vibration & Anomaly Detection with TinyML on Accelerometers","Applications",["anomaly-detection","sensors"]),
   ("TinyML for Computer Vision: Person Detection on a Cortex-M7","Applications",["vision","person-detection"]),
   ("Neural Network Accelerators in Modern MCUs: Ethos-U & MVE","CMSIS-NN",["ethos-u","mve"]),
   ("On-Device Learning: Federated & Incremental Learning Constraints","Applications",["on-device-learning","federated"]),
   ("Power Profiling ML Inference: Measuring Energy per Inference","Memory & Optimization",["power-profiling","inference"]),
   ("Data Collection & Labeling Pipelines for Embedded Sensor Data","Model Prep",["data-collection","labeling"]),
   ("Model Versioning & OTA-Updating Deployed ML Models","Applications",["model-versioning","ota"]),
   ("Testing & Validating TinyML Models on Real Hardware","Applications",["validation","hardware-testing"]),
   ("Full Review & Project: Keyword Spotting on an Arduino Nano BLE Sense","Review",["review","project"]),
  ],
 },
 "fmea": {
  "title": "PFMEA & DFMEA Daily Log",
  "curriculum": [
   ("FMEA Fundamentals: Purpose, History & the AIAG-VDA Handbook","Foundations",["aiag-vda","handbook"]),
   ("DFMEA vs PFMEA vs FMEA-MSR: Key Differences","Foundations",["dfmea","pfmea"]),
   ("FMEA Team Formation & Scope Definition","Foundations",["team","scope"]),
   ("Function Analysis: Block Diagrams & P-Diagrams","Function Analysis",["function-analysis","p-diagram"]),
   ("Failure Chain: Failure Mode, Effect & Cause Relationships","Failure Analysis",["failure-mode","failure-chain"]),
   ("Severity Rating: AIAG-VDA Scales & Customer Impact","Risk Assessment",["severity","rating"]),
   ("Occurrence Rating: Historical Data & Detection Controls","Risk Assessment",["occurrence","rating"]),
   ("Detection Rating: Current Controls & Test Coverage","Risk Assessment",["detection","rating"]),
   ("RPN vs Action Priority (AP): Old vs New AIAG-VDA Approach","Risk Assessment",["rpn","action-priority"]),
   ("DFMEA Deep Dive: Design Interfaces & Boundary Diagrams","DFMEA",["dfmea","boundary-diagram"]),
   ("DFMEA for Electronics: Component-Level Failure Modes","DFMEA",["dfmea","electronics"]),
   ("PFMEA Deep Dive: Process Flow Diagrams & Process Steps","PFMEA",["pfmea","process-flow"]),
   ("PFMEA for PCB Assembly: SMT, Reflow & Inspection Failure Modes","PFMEA",["pfmea","smt","reflow"]),
   ("Linking DFMEA to PFMEA: Special Characteristics","Linkage",["special-characteristics","linkage"]),
   ("FMEA-MSR: Monitoring & System Response for Safety","FMEA-MSR",["fmea-msr","monitoring"]),
   ("Control Plans: From FMEA to Production Controls","Control Plans",["control-plan"]),
   ("FMEA Software Tools: APIS IQ-FMEA & Excel Templates","Tools",["tools","apis-iq-fmea"]),
   ("Integrating FMEA with ISO 26262 / IEC 61508 Hazard Analysis","Integration",["iso26262","iec61508"]),
   ("Common FMEA Mistakes & Avoiding Generic Failure Modes","Best Practices",["best-practices","pitfalls"]),
   ("Full Review & Project: DFMEA/PFMEA for an Embedded Sensor Module","Review",["review","project"]),
  ],
 },
 "fmea-rca": {
  "title": "Root Cause Analysis & 8D Problem Solving Daily Log",
  "curriculum": [
   ("Why Root Cause Analysis? From Symptom to Systemic Fix","Foundations",["rca","root-cause"]),
   ("The 8D Process: Structure & When to Use It","8D",["8d","process"]),
   ("D1-D2: Forming a Team & Defining the Problem Precisely","8D",["d1","d2","problem-definition"]),
   ("D3: Interim Containment Actions","8D",["d3","containment"]),
   ("5 Whys: Technique, Pitfalls & Avoiding Shallow Answers","RCA Techniques",["5-whys"]),
   ("Fishbone/Ishikawa Diagrams: Categorizing Potential Causes","RCA Techniques",["fishbone","ishikawa"]),
   ("Fault Tree Analysis for Root Cause Investigation","RCA Techniques",["fault-tree","rca"]),
   ("Is/Is-Not Analysis: Bounding the Problem Scope","RCA Techniques",["is-is-not"]),
   ("D4: Root Cause Identification & Verification","8D",["d4","root-cause"]),
   ("D5-D6: Permanent Corrective Actions & Validation","8D",["d5","d6","corrective-action"]),
   ("D7: Preventing Recurrence — Updating FMEA & Control Plans","8D",["d7","prevention"]),
   ("D8: Closure & Team Recognition","8D",["d8","closure"]),
   ("Field Failure Analysis: Returned Unit Triage for Firmware/Hardware","Field Analysis",["field-failure","triage"]),
   ("Statistical Process Control (SPC) for Catching Issues Early","Prevention",["spc","prevention"]),
   ("8D Reports: Writing for Customers & Auditors","Documentation",["8d-report","documentation"]),
   ("Common RCA Anti-Patterns: Stopping at the First Plausible Cause","Best Practices",["anti-patterns"]),
   ("Integrating RCA into CAPA (Corrective & Preventive Action) Systems","Integration",["capa","integration"]),
   ("Full Review & Project: 8D Report for a Field-Returned Embedded Device","Review",["review","project"]),
  ],
 },
 "pcb-design": {
  "title": "PCB Design Fundamentals Daily Log",
  "curriculum": [
   ("PCB Design Fundamentals: Layers, Copper Weight & Stackup Basics","Foundations",["pcb","stackup"]),
   ("Schematic Capture Best Practices: Nets, Hierarchical Sheets & Annotations","Foundations",["schematic","nets"]),
   ("Component Footprints & Land Patterns: IPC-7351 Standards","Foundations",["footprints","ipc-7351"]),
   ("PCB Stackup Design: 2-Layer to 8-Layer Boards","Stackup",["stackup","layers"]),
   ("Ground Plane & Return Path Design","Grounding",["ground-plane","return-path"]),
   ("Power Distribution Network (PDN) Design & Decoupling Strategy","Power Integrity",["pdn","decoupling"]),
   ("Thermal Design: Copper Pours, Thermal Vias & Heat Sinking","Thermal",["thermal","copper-pour"]),
   ("Component Placement Strategy: Grouping, Orientation & Test Points","Layout",["placement","test-points"]),
   ("Via Types: Through-Hole, Blind, Buried & Via-in-Pad","Layout",["vias","via-in-pad"]),
   ("Panelization & Fabrication Panel Design","Manufacturing",["panelization"]),
   ("DFM/DFA: Design for Manufacturing & Assembly Checks","Manufacturing",["dfm","dfa"]),
   ("Design Rule Checks (DRC): Common PCB Layout Errors","Verification",["drc","layout-errors"]),
   ("PCB Fabrication Notes: Gerbers, Drill Files & Stackup Specs","Manufacturing",["gerbers","drill-files"]),
   ("Flex & Rigid-Flex PCB Design Basics","Advanced",["flex-pcb","rigid-flex"]),
   ("Connectors & Mechanical Considerations in PCB Layout","Mechanical",["connectors","mechanical"]),
   ("Silkscreen, Assembly Drawings & Fab Notes","Documentation",["silkscreen","assembly-drawing"]),
   ("Pre-Compliance EMI/EMC Considerations at the Layout Stage","Compliance",["emi","emc"]),
   ("Full Review & Project: Layout a 4-Layer Sensor Board from Schematic to Fab","Review",["review","project"]),
  ],
 },
 "high-speed-design": {
  "title": "High-Speed Digital Design Daily Log",
  "curriculum": [
   ('What Makes a Signal "High-Speed"? Rise Time vs Clock Frequency',"Foundations",["high-speed","rise-time"]),
   ("Transmission Line Theory: When Traces Become Transmission Lines","Foundations",["transmission-line"]),
   ("Controlled Impedance: Microstrip & Stripline Calculations","Signal Integrity",["impedance","microstrip"]),
   ("Reflections & Termination Strategies: Series, Parallel & AC Termination","Signal Integrity",["termination","reflections"]),
   ("Differential Pairs: USB, LVDS, PCIe Routing Rules","High-Speed",["differential-pairs","routing"]),
   ("Length Matching & Skew Budget Analysis","High-Speed",["length-matching","skew"]),
   ("Via Stubs & Backdrilling for High-Speed Signals","High-Speed",["via-stubs","backdrilling"]),
   ("Crosstalk: Near-End & Far-End Coupling Mitigation","Signal Integrity",["crosstalk","coupling"]),
   ("Power Integrity: Decoupling Capacitor Placement & PDN Impedance","Power Integrity",["power-integrity","decoupling"]),
   ("Signal Integrity Simulation: S-Parameters & Eye Diagrams","Simulation",["s-parameters","eye-diagram"]),
   ("DDR Routing: Fly-by Topology & Length Matching Rules","High-Speed Memory",["ddr","fly-by"]),
   ("High-Speed Serial Links: SerDes Basics (PCIe, SATA, Ethernet)","High-Speed",["serdes","pcie"]),
   ("Clock Distribution for High-Speed Systems: Skew & Jitter","Clocking",["clock-distribution","jitter"]),
   ("Return Path Discontinuities: Plane Splits & Stitching Vias","Signal Integrity",["return-path","stitching-vias"]),
   ("Simultaneous Switching Noise (SSN) & Ground Bounce","Signal Integrity",["ssn","ground-bounce"]),
   ("Test & Measurement: TDR, VNA & Eye Diagram Capture","Verification",["tdr","vna"]),
   ("EMI Considerations in High-Speed Layout","Compliance",["emi","high-speed"]),
   ("Full Review & Project: Route a DDR4 Fly-by Topology with Length Matching","Review",["review","project"]),
  ],
 },
 "rf-design": {
  "title": "RF Design for Embedded Systems Daily Log",
  "curriculum": [
   ("RF Fundamentals: Frequency, Wavelength & the RF Spectrum for Embedded","Foundations",["rf","spectrum"]),
   ("RF PCB Materials: Dielectric Constant, Loss Tangent & Substrate Choice","Materials",["dielectric","loss-tangent"]),
   ("RF Transmission Lines: Microstrip, Stripline & Grounded CPW","Transmission Lines",["microstrip","cpw"]),
   ("Smith Chart Fundamentals & Impedance Matching","Matching",["smith-chart","impedance-matching"]),
   ("Matching Networks: L-Match, Pi-Match & T-Match Design","Matching",["l-match","pi-match"]),
   ("S-Parameters: Understanding S11, S21 & Return Loss","Characterization",["s-parameters","return-loss"]),
   ("Antenna Basics: Monopole, PIFA & Chip Antennas for Embedded","Antennas",["antenna","pifa"]),
   ("Antenna Placement & PCB Keep-Out Zones","Antennas",["antenna-placement","keep-out"]),
   ("RF Front-End Design: LNAs, PAs & Filters","Front-End",["lna","pa","filters"]),
   ("Balun Design & Single-Ended to Differential Conversion","Front-End",["balun"]),
   ("Shielding & Isolation for RF/Digital Coexistence","Isolation",["shielding","isolation"]),
   ("BLE/Wi-Fi/Sub-GHz RF Layout Guidelines for SoC Modules","Applied",["ble","wifi","sub-ghz"]),
   ("RF Simulation Tools: EM Simulators & Field Solvers","Simulation",["em-simulation","field-solver"]),
   ("RF Measurement: Spectrum Analyzers & VNA Calibration","Measurement",["spectrum-analyzer","vna"]),
   ("Regulatory Certification: FCC/CE/ETSI RF Compliance Basics","Compliance",["fcc","ce","etsi"]),
   ("Harmonics & Spurious Emissions: Causes & Mitigation","Compliance",["harmonics","spurious-emissions"]),
   ("Debugging RF Issues: Detuning, Desense & Coexistence Problems","Debugging",["detuning","desense"]),
   ("Full Review & Project: Design & Match an Antenna Front-End for a BLE Module","Review",["review","project"]),
  ],
 },
 "kicad": {
  "title": "KiCad Design Daily Log",
  "curriculum": [
   ("KiCad Overview: Project Structure & Workflow","Foundations",["workflow","eda"]),
   ("Schematic Editor (Eeschema): Symbols, Wires & Buses","Schematic",["eeschema","symbols"]),
   ("Creating Custom Symbols in KiCad Symbol Editor","Schematic",["symbol-editor","custom-symbols"]),
   ("Hierarchical Sheets & Multi-Sheet Schematics","Schematic",["hierarchical-sheets"]),
   ("ERC (Electrical Rules Check): Catching Schematic Errors","Schematic",["erc"]),
   ("Footprint Editor: Custom Footprints & 3D Models","Footprints",["footprint-editor","3d-models"]),
   ("Assigning Footprints & Netlist Generation","Footprints",["netlist","footprint-assignment"]),
   ("PCB Editor (Pcbnew): Layers, Zones & Board Setup","PCB Editor",["pcbnew","zones"]),
   ("Interactive Router: Push & Shove, Diff Pair Routing","PCB Editor",["interactive-router","diff-pair"]),
   ("Design Rules in KiCad: Netclasses, Clearances & Constraints","PCB Editor",["netclasses","design-rules"]),
   ("Copper Pours & Zone Priorities","PCB Editor",["copper-pour","zones"]),
   ("KiCad Libraries: Managing Global vs Project Libraries","Libraries",["libraries"]),
   ("Plugin & Scripting: KiCad Python API","Automation",["python-api","scripting"]),
   ("Custom Design Rule Checks with KiCad's DRC Engine","Verification",["drc"]),
   ("3D Viewer & Mechanical Fit Checks","Verification",["3d-viewer","mechanical-fit"]),
   ("Gerber & Drill File Generation for Fabrication","Manufacturing",["gerbers","drill-files"]),
   ("BOM Generation & Integration with JLCPCB/PCBWay","Manufacturing",["bom","jlcpcb"]),
   ("Version Control for KiCad Projects: Git Workflow & KiCad-diff","Workflow",["git","version-control"]),
   ("Simulation with ngspice Integration in KiCad","Simulation",["ngspice","simulation"]),
   ("Full Review & Project: Design a Complete Board in KiCad from Schematic to Gerbers","Review",["review","project"]),
  ],
 },
 "rust-zephyr-nrf54": {
  "title": "Rust + Zephyr on nRF54LM20 Daily Log",
  "curriculum": [
   ("nRF54LM20 Overview: Architecture, Cores & Peripherals","Foundations",["nrf54lm20","architecture"]),
   ("Zephyr on Nordic SoCs: nrfx HAL & Devicetree Overlays","Foundations",["zephyr","nrfx"]),
   ("Setting Up Rust + Zephyr: zephyr-rust Crate & Toolchain","Toolchain",["zephyr-rust","toolchain"]),
   ("Building & Flashing a Rust Zephyr App for nRF54LM20","Toolchain",["build","flash"]),
   ("Devicetree Bindings in Rust: Accessing Zephyr DT Nodes","Devicetree",["devicetree","rust"]),
   ("GPIO & LED Control in Rust on Zephyr","Peripherals",["gpio","zephyr"]),
   ("UART Communication: Async Rust Zephyr Drivers","Peripherals",["uart","async"]),
   ("SPI & I2C Peripheral Access in Rust on nRF54LM20","Peripherals",["spi","i2c"]),
   ("Zephyr Kernel Objects from Rust: Threads, Semaphores & Mutexes","Kernel",["threads","semaphores"]),
   ("Interrupt Handling: Rust ISRs in a Zephyr Context","Interrupts",["interrupts","isr"]),
   ("Power Management: nRF54LM20 Low Power Modes with Zephyr PM","Power",["power-management","zephyr-pm"]),
   ("BLE on nRF54LM20: SoftDevice Controller & Rust Bindings","Wireless",["ble","softdevice"]),
   ("Zephyr Bluetooth Host Stack from Rust: GATT Services","Wireless",["bluetooth","gatt"]),
   ("Matter/Thread Support on nRF54LM20 with Zephyr","Wireless",["matter","thread"]),
   ("Memory Safety at the Boundary: Rust/C FFI in Zephyr Modules","FFI",["ffi","memory-safety"]),
   ("Logging & Debugging: defmt/RTT with Zephyr on nRF54LM20","Debugging",["defmt","rtt"]),
   ("Unit Testing Rust Zephyr Code: Twister & Host Tests","Testing",["twister","testing"]),
   ("OTA/DFU on nRF54LM20: MCUboot Integration with Rust Apps","OTA",["mcuboot","dfu"]),
   ("Security Features: TrustZone-M & Rust on nRF54LM20","Security",["trustzone-m","security"]),
   ("Full Review & Project: BLE Sensor Node in Rust on Zephyr/nRF54LM20","Review",["review","project"]),
  ],
 },
 "circuit-design": {
  "title": "Electronics Circuit Design Daily Log",
  "curriculum": [
   ("Circuit Design Fundamentals: Ohm's Law, KVL & KCL in Practice","Foundations",["ohms-law","kvl-kcl"]),
   ("Op-Amp Circuits: Inverting, Non-Inverting & Buffer Configurations","Analog",["op-amp","buffer"]),
   ("Op-Amp Applications: Comparators, Integrators & Differentiators","Analog",["comparator","integrator"]),
   ("Power Supply Design: Linear Regulators (LDO) Basics","Power",["ldo","linear-regulator"]),
   ("Switching Regulators: Buck, Boost & Buck-Boost Topologies","Power",["buck","boost"]),
   ("Power Supply Design: Loop Stability & Compensation","Power",["loop-stability","compensation"]),
   ("Battery Charging Circuits: Li-Ion/LiPo Charge Management ICs","Power",["li-ion","charging"]),
   ("Filter Design: RC, RL & Active Filters","Analog",["filters","rc"]),
   ("Analog-to-Digital Conversion: ADC Types & Sampling Theory","Mixed-Signal",["adc","sampling"]),
   ("Digital-to-Analog Conversion: DAC Architectures & Applications","Mixed-Signal",["dac"]),
   ("Sensor Signal Conditioning: Amplification & Noise Reduction","Analog",["signal-conditioning","noise"]),
   ("Logic Level Translation & Interfacing 3.3V/5V/1.8V Devices","Digital",["level-shifting","logic"]),
   ("Digital Logic Design: Combinational vs Sequential Circuits","Digital",["combinational","sequential"]),
   ("Clock Distribution & Crystal Oscillator Circuit Design","Digital",["crystal","oscillator"]),
   ("ESD Protection: TVS Diodes & Layout Considerations","Protection",["esd","tvs-diode"]),
   ("Reverse Polarity & Overcurrent Protection Circuits","Protection",["reverse-polarity","overcurrent"]),
   ("Motor Driver Circuits: H-Bridges & Gate Drivers","Actuators",["h-bridge","gate-driver"]),
   ("MOSFET Selection & Switching Circuit Design","Power",["mosfet","switching"]),
   ("Circuit Simulation: SPICE Basics & Model Selection","Simulation",["spice","simulation"]),
   ("Thermal Design for Circuits: Derating & Heat Dissipation","Thermal",["derating","thermal"]),
   ("Debugging Analog Circuits: Oscilloscope & Multimeter Techniques","Debugging",["oscilloscope","multimeter"]),
   ("Full Review & Project: Design a Battery-Powered Sensor Front-End","Review",["review","project"]),
  ],
 },
}
...
def generate_post(topic_key: str, day: int = None, dry_run: bool = False) -> None:
    ...
    if day > len(curriculum):
        print(f"  [{topic_key}] Curriculum complete ({len(curriculum)} days) — skipping day {day}")
        return
